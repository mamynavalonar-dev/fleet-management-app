...
const LogbookPage = () => {
  ...
  const fetchLogbook = async () => {
    setLoading(true);
    try {
      // On passe le paramètre 'deleted' pour savoir si on veut la corbeille ou non
  ...
  const handleEdit = (tripId, field, value) => {
  ...
  const handleNewTrip = async (e) => {
    e.preventDefault();
    try {
      await client.post('/trips', { ...newTripData, vehicle_id: id, km_depart: parseFloat(newTripData.km_depart), km_arrivee: parseFloat(newTripData.km_arrivee) });
      ...
      <div className={`printable-area bg-white shadow-xl max-w-[297mm] mx-auto p-8 min-h-[210mm] border border-gray-200 text-sm ${viewTrash ? 'border-red-400 bg-red-50' : ''}`}>
        ...
        <div className="mb-8">
            ...
            <table className="w-full border-collapse border border-gray-300 text-xs">
                ...
                <tbody>
                    {trips.length === 0 ? (
                        <tr><td colSpan="7" className="p-8 text-center text-gray-400 italic">Aucune donnée.</td></tr>
                    ) : (
                        trips.map((trip) => (
                            <tr key={trip.id} className="text-center hover:bg-gray-50 group">
                                ...
                                <td className="border border-gray-300 p-2 font-bold">
                                    {trip.km_arrivee && trip.km_depart ? (trip.km_arrivee - trip.km_depart).toFixed(0) : '-'}
                                </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="border border-gray-300 p-4 bg-gray-50 rounded-lg">
                <div className="flex justify-between items-center mb-4"><span className="text-gray-600">Total KM :</span><span className="text-xl font-bold">{trips.reduce((acc, t) => acc + (t.km_arrivee - t.km_depart), 0)} km</span></div>
                <div className="flex justify-between items-center mb-8"><span className="text-gray-600">Conso Moyenne :</span><span className="text-lg font-bold text-purple-600">{((fuels.reduce((acc, f) => acc + Number(f.litres), 0) / trips.reduce((acc, t) => acc + (t.km_arrivee - t.km_depart), 0)) * 100 || 0).toFixed(1)} L/100</span></div>
